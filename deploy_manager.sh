...
#
# 更新日志:
# - [重大纠正] 彻底废除调用外部脚本卸载 MosDNS 的错误做法。
# - [新增] 实现了一个内置的、可靠的 uninstall_mosdns_manually 函数，
#            通过手动停止服务、删除文件的方式，确保强制、干净地移除 MosDNS 组件。
# - 这从根本上解决了脚本在清理阶段反而错误地安装 MosDNS 的严重问题。
#
# ========================================================================================
# --- 配置 ---
...
uninstall_previous_services() {
    ...
    if [[ -f "/etc/init.d/sniproxy" || -f "/etc/systemd/system/sniproxy.service" ]]; then
        ...
        if ! wget --no-check-certificate -q -O "$installer_name" "${GITHUB_PROXY}${DNSMASQ_SNIPROXY_INSTALLER_URL}"; then
            warn "下载卸载脚本失败。"
        else
    ...
    if [[ -d "/etc/mosdns" || -f "/usr/local/bin/mosdns" || -f "/etc/systemd/system/mosdns.service" ]]; then
        info "检测到 MosDNS 存在，将执行内置的强制清理程序..."
        uninstall_mosdns_manually
    fi
}
pre_flight_checks() {
...
run_main_installation() {
    ...
    local installer_name="dnsmasq_sniproxy.sh"; info "正在通过代理下载主安装脚本..."
    if ! wget --no-check-certificate -q -O "$installer_name" "${GITHUB_PROXY}${DNSMASQ_SNIPROXY_INSTALLER_URL}"; then error "通过代理下载主安装脚本失败！"; fi
    
    info "正在动态修改子脚本，为其添加下载代理..."; sed -i -E "s#(https://raw.githubusercontent.com)#${GITHUB_PROXY}\1#g" "$installer_name"; info "子脚本修复完成！"
