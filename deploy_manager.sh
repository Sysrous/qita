...
pre_flight_checks() {
    ...
    install_tool "wget"
    install_tool "curl"
    install_tool "chattr" "e2fsprogs"
    install_tool "jq" # 确保 jq 已安装
...
run_main_installation() {
    ...
    if [ ${exit_code} -ne 0 ]; then
        error "主脚本执行失败 (退出码: ${exit_code})。"
    fi
    if ! check_port 53; then
        error "安装后检测失败：53端口未被 Dnsmasq 监听。"
    fi
    info "核心服务安装并运行成功！"
    rm -f "$installer_name"
}
...
apply_whitelist() {
    ...
    if ! wget --no-check-certificate -q -O "$installer_name" "$WHITELIST_INSTALLER_URL"; then
        error "下载白名单脚本失败: $WHITELIST_INSTALLER_URL"
    fi
    info "开始执行白名单脚本..."
    echo "--- [白名单脚本输出开始] ---"
    bash "$installer_name"
...
set_local_dns_resolver() {
    step 5 "配置系统DNS解析"
    info "正在将本机DNS永久指向 127.0.0.1 ..."
    unprotect_resolv_conf
    if [ -d /etc/netplan ] && ls /etc/netplan/*.yaml &>/dev/null; then
        info "检测到 Netplan 配置..."
        for file in /etc/netplan/*.yaml; do cp "$file" "${file}.bak_$(date +%F)"; done
        ...
        info "检测到 systemd-resolved 服务..."
        cp /etc/systemd/resolved.conf /etc/systemd/resolved.conf.bak_$(date +%F)
        sed -i -E 's/^#?DNS=.*/DNS=127.0.0.1/' /etc/systemd/resolved.conf
        sed -i -E 's/^#?FallbackDNS=.*/#FallbackDNS=/' /etc/systemd/resolved.conf
        systemctl restart systemd-resolved
        info "systemd-resolved 配置完成。"
    elif command -v nmcli &> /dev/null && systemctl is-active --quiet NetworkManager; then
        info "检测到 NetworkManager..."
        active_con=$(nmcli -t -f NAME,DEVICE connection show --active | head -n1 | cut -d: -f1)
        if [ -n "$active_con" ]; then
            info "正在修改活动连接: '$active_con'"
            nmcli con mod "$active_con" ipv4.dns "127.0.0.1"
            nmcli con mod "$active_con" ipv4.ignore-auto-dns yes
            nmcli con up "$active_con" >/dev/null
            info "NetworkManager 配置完成。"
        else
            warn "未找到活动的 NetworkManager 连接。"
        fi
    else
        info "未检测到主流网络管理工具，将直接修改 /etc/resolv.conf"
        warn "警告: 直接修改此文件可能不是永久性的。"
        cp /etc/resolv.conf /etc/resolv.conf.bak_$(date +%F)
        echo "nameserver 127.0.0.1" > /etc/resolv.conf
        info "/etc/resolv.conf 已修改。"
    fi
    protect_resolv_conf
...
configure_xrayr() {
    ...
    # 我们的 Dnsmasq 服务固定监听在 53 端口
    local TARGET_DNS_PORT="53"
    ...
    if [ -f "$ROUTE_FILE" ]; then
        ...
        jq --argjson port "$TARGET_DNS_PORT" '
            .rules |= ([{
                "type": "field",
                "ip": ["127.0.0.1"],
                "port": $port,
    ...
    # --- 2. 修改 dns.json (包含端口修正逻辑) ---
    if [ -f "$DNS_FILE" ]; then
        info "正在处理 $DNS_FILE ..."
        local tmp_dns=$(mktemp)
        # 逻辑:
        # 1. 强制将 127.0.0.1:53 作为首选DNS。
        # 2. 保留所有带 "domains" 的解锁规则。
        # 3. 在保留的解锁规则中，如果端口是 10053，则自动修正为 53。
        # 4. 丢弃所有其他不带 "domains" 的通用DNS (如 8.8.8.8)。
        jq --argjson target_port "$TARGET_DNS_PORT" '
            .servers = ([{
                "address": "127.0.0.1",
                "port": $target_port
            }] + (
                .servers | map(
                    ...
                    (.port |= if . == 10053 then $target_port else . end)
                )
            ))
